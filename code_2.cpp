    ...
    while(1)
    {
        ...
        while(mousemsg())
        {
            ...
            if(lick.x >= 404 && lick.x <= 622 && lick.y >= 179 && lick.y <= 266)        ///获取鼠标点击位置
            {
                if(lick.is_down())
                {
                    putimage(622, 175, enterchange[2]);                           ///点击板块图像变化
                    Sleep(300);
                    flag = 1;
                }
                if(lick.is_up())
            ...
            else if(lick.x >= 405 && lick.x <= 622 && lick.y >= 299 && lick.y <= 385)
            {
                if(lick.is_down())
                {
                    putimage(309, 300, enterchange[0]);
                    Sleep(300);
                    flag = 3;
                }
                if(lick.is_up())
            ...
            else if(lick.x >= 404 && lick.x <= 622 && lick.y >= 424 && lick.y <= 500)
            {
                if(lick.is_down())
                {
                    putimage(622, 415, enterchange[1]);
                    Sleep(300);
                    flag = 5;
                }
                if(lick.is_up())
...
void data_withoutinput(int paotime)  ///没有数据输入时
{
